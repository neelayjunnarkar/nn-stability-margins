...
class InvertedPendulumEnv(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        # self.max_reward = 3
        self.max_reward = self.max_torque**2 # 4*self.max_pos**2 + 4*self.max_speed**2 + self.max_torque**2
        # self.states = []
    ...
    def step(self, u, fail_on_state_space = True, fail_on_time_limit = True):
        ...
        # Js = 1*th**2 + 0.1*thdot**2 + 10*(u[0]*self.factor)**2
        # max_Js = 1*self.max_pos**2 + 0.1*self.max_speed**2 + 10*self.max_torque**2
    ...
    def reset(self, state = None):
        if state is None:
            ...
            # high = np.array([0.55 * self.max_pos, 0.01 * self.max_speed], dtype=np.float32) * self.factor
            high = np.array([0.6 * self.max_pos, 0.01 * self.max_speed], dtype=np.float32) * self.factor
            # high = np.array([0.2 * self.max_pos, 0.5 * self.max_speed], dtype=np.float32) * self.factor
